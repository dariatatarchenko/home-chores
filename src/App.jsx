    ...
    <div style={{height:"100%",background:"#08080f",display:"flex",justifyContent:"center",alignItems:"stretch",fontFamily:"'SF Pro Text',-apple-system,system-ui,sans-serif",overflow:"hidden"}}>
      ...
      <div style={{width:"100%",maxWidth:480,overflow:"hidden",display:"flex",flexDirection:"column",height:"100%",position:"relative",background:tc.bg}}>
          ...
          {tab==="tasks"&&(
            <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                ...
                <button onClick={()=>setShowStats(true)} style={{position:"relative",display:"flex",alignItems:"center",gap:6,height:40,boxSizing:"border-box",background:"rgba(251,191,36,0.15)",border:"none",borderRadius:20,padding:"0 16px",color:"#fbbf24",fontSize:14,fontWeight:500,cursor:"pointer"}}>
                  <div style={{position:"absolute",inset:0,borderRadius:20,border:"1.5px solid rgba(251,191,36,0.4)",pointerEvents:"none"}}/>
        ...
        <div style={{position:"absolute",left:8,right:8,bottom:24,zIndex:100,display:"flex",alignItems:"center",gap:8}}>
          <div key={theme} style={{flex:1,height:64,boxSizing:"border-box",position:"relative",background:isDark?"rgba(78,82,135,0.5)":"rgba(255,255,255,0.6)",backdropFilter:"blur(24px) saturate(120%)",WebkitBackdropFilter:"blur(24px) saturate(120%)",border:isDark?"1px solid #494D68":"1px solid rgba(255,255,255,1)",borderRadius:32,padding:2,boxSizing:"border-box",display:"flex",alignItems:"center",justifyContent:"center",gap:0}}>
            {TABS.filter(item=>!item.accent).map(item=>{
              const active=tab===item.id&&!taskFormOpen;
              return (
                <button key={item.id} onClick={()=>{setTaskFormOpen(false);setTab(item.id);}} style={{position:"relative",flex:"1 1 82px",minWidth:82,height:60,border:"none",padding:10,boxSizing:"border-box",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",background:"transparent"}}>
          ...
          <button onClick={()=>{setTaskNameError(false);setAssigneeError(false);setEditTaskId(null);setForm(blankForm);setCustomTimeOpen(false);setTaskFormOpen(true);}} style={{flexShrink:0,width:48,height:48,borderRadius:24,border:"none",background:"linear-gradient(135deg,#5EF9B0,#6388FF,#7B61FF)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
